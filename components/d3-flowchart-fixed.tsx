...
export function D3Flowchart({ diagram, diagramType }: D3FlowchartProps) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Center the graph
      const svgWidth = parseInt(svg.attr('width') || '800')
  ...
  function parseMermaidToFlowData(mermaidString: string): FlowData {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (line.includes('-->') || line.includes('--')) {
        ...
        // Handle different edge syntaxes
        if (line.includes('-->')) {
          ...
          if (parts.length >= 3) {
            source = parts[0].trim()
            label = parts[1].trim().replace(/^\||\|$/g, '') // Remove | characters
            target = parts[2].trim()
          }
        }
        ...
        if (source && target) {
          ...
          if (!nodes.some(n => n.id === source)) {
            nodes.push({
              ...
              data: { label: extractNodeLabel(source) }
            })
          }
          ...
          if (!nodes.some(n => n.id === target)) {
            nodes.push({
              ...
              data: { label: extractNodeLabel(target) }
            })
          }
